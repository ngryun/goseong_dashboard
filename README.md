...
- 같은 시트를 두 번 등록하면 한 번만 읽습니다. 월간 달력에는 월중행사 제목만 표시하고 주간업무는 날짜별 건수 버튼으로 표시합니다. 건수 버튼을 누르면 해당 날짜의 주간업무 목록이 열립니다. 월간 화면은 일정 구분별로 같은 날짜·제목(공백 차이 제외)을 묶으며, 상세 화면에서 모든 담당·시간·출처를 확인할 수 있습니다. 주간 표와 전체 목록, 상단 요약 건수는 원본 항목 기준입니다.

...
function monthChip(e){
  ...
  return `<button class="event-chip month-event" data-event="${esc(e.id)}" data-merged="true" title="${esc(e.title)}"><span class="month-event-title">${esc(e.title)}</span>${time?`<span class="month-event-time">${esc(time)}</span>`:''}</button>`;
}
function renderMonth(events){
  ...
  for(let i=0;i<cells;i++){
    ...
    html+=`<div class="day ${d.getMonth()!==cursor.getMonth()?'outside':''} ${selected===key?'selected':''}"><button class="day-number ${key===todayKey?'today':''}" data-date="${key}" aria-pressed="${selected===key}" aria-label="${key} 월중행사 ${items.monthly.length}건, 주간업무 ${items.weekly.length}건">${d.getDate()}</button><div class="month-events">${items.monthly.slice(0,3).map(monthChip).join('')}${items.monthly.length>3?`<button class="more" data-date="${key}" data-day-kind="monthly">행사 +${items.monthly.length-3}건</button>`:''}</div>${items.weekly.length?`<button class="weekly-count" data-date="${key}" data-day-kind="weekly" aria-label="${key} 주간업무 ${items.weekly.length}건 보기"><span>주간업무</span><strong>${items.weekly.length}건</strong><span aria-hidden="true">›</span></button>`:''}</div>`;
  }
...
function renderDay(events){
  ...
  $('#day-events').innerHTML=items.map(e=>{
    ...
    const teams=values(x=>x.team||kindLabel(x)),times=values(x=>x.time||'시간 미기재'),places=values(x=>x.place),owners=values(x=>x.owner);
    return `<button class="day-event category-${groupFor(e)}" data-event="${esc(e.id)}" ${view==='month'?'data-merged="true"':''}><span class="badge ${e.kind} category-badge">${esc(teams)}</span><h3>${esc(e.title)}</h3><div class="event-meta">◷ ${esc(times)}${places?'<br>⌖ '+esc(places):''}${owners?'<br>'+esc(owners):''}${members.length>1?'<br>원본 '+members.length+'건 보기':''}</div></button>`;
...
if(multiple){
  ...
  $('#detail-related').innerHTML=members.map(x=>`<article class="related-event"><h3>${esc(x.team||kindLabel(x))}</h3><p>${esc([x.time||'시간 미기재',x.place,x.owner].filter(Boolean).join(' · '))}</p><pre>${esc(x.description||x.title)}</pre><a href="${esc(sourceUrl(x))}" target="_blank" rel="noopener">${esc(x.tab)} · ${esc(x.cell)} 원문 보기 ↗</a></article>`).join('');

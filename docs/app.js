...
// data.events is always the union of sheet events (data.json) and events typed into the page (Firestore).
let sheetEvents = [], fsEvents = [];
...
const LEVEL_MARK = {elementary:'초', middle:'중', high:'고'};
...
function schoolFlag(g){
  const n=!schoolCode&&g.members.length>1?`<small>${g.members.length}교</small>`:'';
  return `<button class="day-flag type-${typeClass(g.type)}" data-school-group="${esc(g.key)}" title="${esc(g.title)}">${esc(g.title)}${n}</button>`;
}
function renderSchool(){
  ...
  html+='</div></div>';
  const timetable=school?`<section class="timetable-section"><div class="timetable-head"><h3>주간 시간표</h3><span id="timetable-week"></span></div><div id="timetable"></div></section>`:'';
...
$('#detail').showModal();}
document.addEventListener('click',e=>{const ed=e.target.closest('[data-edit]');if(ed){$('#detail').close();openEditor(ed.dataset.edit);return;}const add=e.target.closest('[data-add-date]');if(add){selected=add.dataset.addDate;render(true);openEditor();return;}const dayControl=e.target.closest('[data-day-kind]');if(dayControl){dayKind=dayControl.dataset.dayKind;if(dayControl.dataset.date)selected=dayControl.dataset.date;render(true);$('#day-events').scrollTop=0;if(dayControl.dataset.date){$('#selected-title').focus({preventScroll:true});if(window.matchMedia('(max-width:850px)').matches)$('.day-panel').scrollIntoView({behavior:'smooth',block:'start'});}return;}const group=e.target.closest('[data-group]');if(group){activeGroup=group.dataset.group;$('#team').value='';$('#kind').value='';render();return;}const sg=e.target.closest('[data-school-group]');if(sg)return showSchoolDetail(sg.dataset.schoolGroup);const lv=e.target.closest('[data-level]');if(lv){schoolLevel=lv.dataset.level;schoolCode='';render(true);return;}const cl=e.target.closest('[data-class]');if(cl&&lastTimetable){classChoice.set(lastTimetable.school.code,cl.dataset.class);drawTimetable(lastTimetable.school,lastTimetable.start,lastTimetable.rows);return;}const ev=e.target.closest('[data-event]');if(ev)return showDetail(ev.dataset.event,ev.dataset.merged==='true');const d=e.target.closest('[data-date]');if(d){selected=d.dataset.date;dayKind=$('#kind').value||'monthly';render(true);return;}const cell=e.target.closest('.day[data-day]');if(cell&&!e.target.closest('button')){selected=cell.dataset.day;dayKind=$('#kind').value||'monthly';render(true);return;}const v=e.target.closest('[data-view]');if(v){view=v.dataset.view;activeGroup='';dayKind=$('#kind').value||'monthly';render();}});
...
const FIREBASE = {apiKey:'AIzaSyAbx1GNaj7qpduIAGgA6M1CUsSrRZFY0zs', authDomain:'goseong-calendar.firebaseapp.com', projectId:'goseong-calendar', appId:'1:358512557737:web:1199a41cd8078fa75df3b8'};
...
async function connectFirestore(){
  try{
    ...
    const db=f.getFirestore(initializeApp(FIREBASE));fb={...f,db};
    f.onSnapshot(f.query(f.collection(db,'events'),f.where('deleted','==',false)),snap=>{
      fsEvents=snap.docs.map(d=>fsEvent(d.id,d.data())).filter(e=>typeof e.date==='string'&&typeof e.title==='string'&&e.title).sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time));
      mergeEvents();render(true);if(data.updated_at)renderSources();
...
function openEditor(id=null){
  ...
  f.date.value=e?e.date:selected;f.kind.value=e?e.kind:($('#kind').value||(view==='month'?dayKind:'weekly'));
  f.title.value=e?e.title:'';f.team.value=e?e.team:($('#team').value||'');f.time.value=e?e.time:'';f.place.value=e?e.place:'';f.owner.value=e?e.owner:'';f.description.value=e?e.description:'';

...
else{$('#period-title').textContent=`${cursor.getFullYear()}년 ${cursor.getMonth()+1}월`;shown=events.filter(e=>e.date.startsWith(month));$('#calendar').innerHTML=shown.map(e=>`<button class="list-row" data-event="${e.id}"><span class="list-date">${Number(e.date.slice(5,7))}.${Number(e.date.slice(8))}${e.time?'<br>'+esc(slotsOf(e)?slotsOf(e).map(s=>s.time).join(' · '):e.time):''}</span><span class="list-info"><strong>${esc(e.title)}</strong><small>${esc([e.team,e.place,e.owner].filter(Boolean).join(' · ')||e.tab)}</small></span><span class="badge ${e.kind}">${kindLabel(e)}</span></button>`).join('')||'<div class="empty"><strong>표시할 일정이 없습니다</strong>조회 기간이나 검색 조건을 변경해 주세요.</div>';}

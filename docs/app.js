...
function renderSchool(){
  ...
  for(let i=0;i<cells;i++){
    ...
    html+=`<div class="day ${d.getMonth()!==cursor.getMonth()?'outside':''} ${selected===key?'selected':''}"><button class="day-number ${key===todayKey?'today':''}" data-date="${key}" aria-pressed="${selected===key}" aria-label="${key} 학사일정 ${items.length}건">${d.getDate()}</button><div class="month-events">${items.slice(0,3).map(schoolChip).join('')}</div><div class="month-day-footer">${items.length>3?`<button class="more" data-date="${key}">+${items.length-3}건 더 보기</button>`:''}</div></div>`;
  }
  html+='</div></div>';

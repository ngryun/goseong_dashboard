...
function monthChip(e){
  ...
  return `<button class="event-chip month-event" data-event="${esc(e.id)}" data-merged="true" title="${esc(e.title)}"><span class="month-event-time">${esc(time||'시간 미기재')}</span><span class="month-event-title">${esc(e.title)}</span></button>`;
}
